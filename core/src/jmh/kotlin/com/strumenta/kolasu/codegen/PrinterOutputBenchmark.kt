...
data class BenchStatement(
    val text: String,
) : Node()

data class BenchFunction(
    val name: String,
    val body: MutableList<BenchStatement> = mutableListOf(),
) : Node()

data class BenchClass(
    val name: String,
    val functions: MutableList<BenchFunction> = mutableListOf(),
) : Node()

data class BenchFile(
    val pkg: String,
    val classes: MutableList<BenchClass> = mutableListOf(),
) : Node()
...
open class PrinterOutputBenchmark {
    ...
    private fun buildAst(
        classCount: Int,
        functionsPerClass: Int,
        statementsPerFunction: Int,
    ): BenchFile {
        val classes =
            (1..classCount)
                .map { ci ->
