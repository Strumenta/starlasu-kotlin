...
class LionWebTreeWalker {
    ...
    fun thisAndAllDescendants(node: LWNode): List<LWNode> {
        ...
        while (stack.isNotEmpty()) {
            ...
            if (current !is ProxyNode) {
                val containments = containmentsCache.computeIfAbsent(current.classifier.id!!) {
    ...
    private fun collectLeavesFirst(node: LWNode, result: ArrayList<LWNode>) {
        if (node is ProxyNode) {
            result.add(node)
            return
        }
        val containments = containmentsCache.computeIfAbsent(node.classifier.id!!) {
